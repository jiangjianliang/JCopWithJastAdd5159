...
public class JCopAspect {
	 ...
	 * @param layer
	 */
	public void addWrapperForField(FieldDeclaration field,
			LayerDeclaration layer) {
		String fqnName = layer.hostType().topLevelType().fullName() + "."
				+ field.name();
		String flattenedName = fqnName.replaceAll("\\.", "_");
		PartialFieldAdviceGenerator gen = new PartialFieldAdviceGenerator(
				advice);
	...
	public void addContextActivations(ContextDecl decl,
		...
		ContextClassAdviceGenerator gen = new ContextClassAdviceGenerator(
				this.context, decl, pointcutSignature);
	 ...
	 */
	public void addLayerActivationStatement(List<ParseName> subjects,
			String layerComposition) {
		SubjectLayerAdviceGenerator gen = new SubjectLayerAdviceGenerator(
				advice);
		gen.genLayerActivationStatement(subjects, layerComposition);
	}

	public void addLayerImport(LayerDeclaration layer) {
		ClassDecl decl = jcop.transformation.lookup.Lookup
				.lookupLayerClassDecl(layer);
	...
	private String[][] globalValues = {
			...
			{ "compositionVarAssignment",
					"$composition$ oldComposition= $currentComposition$" } };
	...
	// Set<String> candidatesSignatures = new
	// HashSet<String>(unvisitedMemberForDelegation);
	// candidatesSignatures.removeAll(visitedMemberForDelegation);
	// SourceCodeBuffer buffer = new SourceCodeBuffer();
	// for (String candidateSignature : candidatesSignatures) {
	// SourceCodeBuffer b =
	// PartialMethodAdviceGenerator.getInterTypeDeclarationsFor(candidateSignature);
	// buffer.add(b);
	// }
	...
	// PartialMethodAdviceGenerator gen = new
	// PartialMethodAdviceGenerator(advice, decl);
	...
	// private void addLayerDecls(AST.List<BodyDecl> bodyDecls, LayerDecl decl,
	// PartialMethodAdviceGenerator gen) {
	...
	// private void addLayerDecl(LayerDecl layerDecl, PartialMethodDecl
	// partialMethod, PartialMethodAdviceGenerator gen) {
	...
	private void addMemberToLayerClass(LayerDeclaration decl,
			PartialMethodDecl method) {
		// LayerClassGenerator gen = new LayerClassGenerator(LayerDecl);
		Generation.removeStaticModifier(method.getModifiers());
		jcop.transformation.lookup.Lookup.lookupLayerClassDecl(decl)
				.addMemberMethod(method);
	}
