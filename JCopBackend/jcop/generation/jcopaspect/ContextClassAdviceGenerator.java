...
class ContextClassAdviceGenerator extends AdviceGenerator {
	...
	public ContextClassAdviceGenerator(SourceCodeBuffer advice,
			ContextDecl decl, PointcutExpr pointcut) {
		super(advice);
		this.decl = decl;
	...
	/**
	 * generate 'jcopLookup' pointcut in JCopAspect.aj this method should only
	 * be called once during a compilation
	 */
	private void addPointcut() {
		getAdvice()
				.put(new CharSequence[] { pointcutDecl(
						"jcopLookup() :",
	...
	public void addContextActivation(String signature) {
		...
		// advice.putLine("// context ", decl.getFullQualifiedName(),
		// " activation for ", signature);
	...
	public void genContextActivation(List<String> executionSignatures) {
		...
		if (Program.hasOption("-rtl")) {
			loggingBlock = block(new CharSequence[] {
